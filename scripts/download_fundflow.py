...
def get_sina_flow(code):
    ...
    try:
        ...
        rename_map = {
            'opendate': 'date', 
            'netamount': 'net_flow_amount', # 主力净流入
            'r0_net': 'main_net_flow',      # 主力 = 超大+大
            'r1_net': 'super_large_net_flow',
            'r2_net': 'large_net_flow',
            'r3_net': 'medium_small_net_flow'
        }
...
def main():
    ...
    for s in tqdm(stocks, desc=f"Job {TASK_INDEX}"):
        df = get_sina_flow(s['code'])
        if not df.empty:
            df['code'] = s['code'] # 补全code便于合并
            df.to_parquet(f"{OUTPUT_DIR}/{s['code']}.parquet", index=False)
        time.sleep(random.uniform(0.1, 0.3)) # 礼貌爬取
